...
Future<void> sendSurveyData(String token, Map<String, dynamic> data) async {
  final url = Uri.parse('https://kooala.tassoo.uk/users/survey');
...
class _SleepSurveyHomeState extends State<SleepSurveyHome> {
  ...
  bool _isCurrentPageValid() {
    switch (_currentPage) {
      ...
      case 3:
        return soundPreference != null &&
            calmingSound != null &&
            deviceUsage != null &&
            autoStopPreference != null;
      case 4:
        return sleepLatency != null &&
            caffeineIntake != null &&
                ...
                onPressed: () async {
                  if (_isCurrentPageValid()) {
                    if (_currentPage < pages.length - 1) {
                      ...
                      final surveyData = {
                        "sleepLightUsage": lightUsage,
                        "lightColorTemperature": lightTone,
                        "noisePreference": noisePreference,
                        "youtubeContentType": youtubeContent,
                        "usualBedtime": sleepTime,
                        "usualWakeupTime": wakeTime,
                        "dayActivityType": activityType,
                        "morningSunlightExposure": sunlightExposure,
                        "napFrequency": napFrequency,
                        "napDuration": napDuration,
                        "mostDrowsyTime": sleepyTime,
                        "averageSleepDuration": avgSleep,
                        "sleepIssues": [sleepProblem!], // 단일 선택이라도 리스트 형태로
                        "emotionalSleepInterference": [emotionalFactor!],
                        "soundPreference": soundPreference,
                        "calmingSound": calmingSound,
                        "deviceUsage": deviceUsage,
                        "autoStopPreference": autoStopPreference,
  ...
  Widget _buildEnvironmentPage() {
    return ListView(
      ...
      children: [
        ...
        _buildRadioGroup(
          'Q2. 사용하는 조명의 색온도는?',
          ['차가운 하얀색 (6500K)', '중간 톤 (4000K)', '따뜻한 노란색 (2700K)', '잘 모르겠음'],
          lightTone,
          (v) => setState(() => lightTone = v),
        ),
        _buildRadioGroup(
          'Q3. 수면 시 주변 소음에 대한 본인의 선호는?',
          ['완전한 무음이 좋아요', '백색소음이 필요해요', '유튜브 등을 틀어두는 편이에요', '기타'],
          noisePreference,
          (v) => setState(() => noisePreference = v),
        ),
        _buildRadioGroup(
          'Q4. 유튜브를 튼다면 어떤 콘텐츠를 듣나요?',
          ['ASMR', '음악 (재즈, 클래식 등)', '라디오/토크 방송', '드라마/영상 콘텐츠', '기타'],
          youtubeContent,
  ...
  Widget _buildHabitRhythmPage() {
    return ListView(
      ...
      children: [
        _buildRadioGroup(
          'Q5. 보통 취침 시간은 언제인가요?',
          ['오후 9시 이전', '오후 9시 ~ 자정', '자정 ~ 새벽 2시', '새벽 2시 이후'],
          sleepTime,
          (v) => setState(() => sleepTime = v),
        ),
        _buildRadioGroup(
          'Q6. 보통 기상 시간은 언제인가요?',
          ['오전 5시 이전', '오전 5시 ~ 7시', '오전 7시 ~ 9시', '오전 9시 이후'],
          wakeTime,
          (v) => setState(() => wakeTime = v),
        ),
        _buildRadioGroup(
          'Q7. 하루 중 어느 활동이 더 많은가요?',
          ['실내 활동이 많아요', '실외 활동이 많아요', '비슷하게 활동해요'],
          activityType,
          (v) => setState(() => activityType = v),
        ),
        _buildRadioGroup(
          'Q8. 평소 아침 햇빛을 쬐는 빈도는?',
          ['거의 매일 쬐어요', '가끔 쬐어요', '거의 쬐지 않아요'],
          sunlightExposure,
          (v) => setState(() => sunlightExposure = v),
        ),
        _buildRadioGroup(
          'Q9. 낮잠을 자는 빈도는?',
          ['매일', '주 3~4회', '가끔 (주 1~2회)', '거의 안 잔다'],
          napFrequency,
          (v) => setState(() => napFrequency = v),
        ),
        _buildRadioGroup(
          'Q10. 보통 낮잠 시간은?',
          ['15분 이내', '15~30분', '30분~1시간', '1시간 이상'],
          napDuration,
          (v) => setState(() => napDuration = v),
        ),
        _buildRadioGroup(
          'Q11. 가장 졸리거나 피곤한 시간대는?',
          ['오전', '오후', '저녁', '새벽', '일정치 않음'],
          sleepyTime,
          (v) => setState(() => sleepyTime = v),
        ),
        _buildRadioGroup(
          'Q12. 최근 1주일 평균 수면 시간은?',
  ...
  Widget _buildSoundPreferencePage() {
    return ListView(
      ...
      children: [
        _buildRadioGroup(
          'Q15. 수면 시 듣기 편한 사운드는?',
          ['자연 소리', '음악', '저주파/백색소음', '목소리 (ASMR 등)', '무음이 가장 편함'],
          soundPreference,
          (v) => setState(() => soundPreference = v),
        ),
        _buildRadioGroup(
          'Q16. 마음을 안정시켜주는 사운드는?',
          ['비 오는 소리', '파도/물소리', '잔잔한 피아노', '사람의 말소리', '기타'],
          calmingSound,
          (v) => setState(() => calmingSound = v),
        ),
        _buildRadioGroup(
          'Q17. 수면 시 사용하는 기기는?',
          ['스마트워치', '스마트폰 앱', '스마트 조명', '사운드 기기 (스피커, 이어폰 등)', '없음'],
          deviceUsage,
          (v) => setState(() => deviceUsage = v),
        ),
        _buildRadioGroup(
          'Q18. 사운드 자동 종료 방식은?',
          ['고정 시간 후 종료', '수면 상태 감지 시 자동 종료', '수동 종료', '사용하지 않음'],
          autoStopPreference,
          (v) => setState(() => autoStopPreference = v),
        ),
      ],
    );
  }
  ...
  Widget _buildHealthLifestylePage() {
    return ListView(
      ...
      children: [
        _buildRadioGroup(
          'Q19. 잠들기까지 걸리는 시간은?',
          ['5분 이내', '5~15분', '15~30분', '30분 이상'],
          sleepLatency,
          (v) => setState(() => sleepLatency = v),
        ),
        _buildRadioGroup(
          'Q20. 카페인 섭취량은?',
          ['안 마심', '하루 1~2잔', '하루 3잔 이상'],
          caffeineIntake,
        ...
        _buildRadioGroup(
          'Q22. 취침 전 전자기기 사용 시간은?',
          ['없음', '30분 미만', '1시간 이상'],
          screenTimeBeforeBed,
          (v) => setState(() => screenTimeBeforeBed = v),
        ),
        _buildRadioGroup(
          'Q23. 최근 스트레스 수준은?',
          ['높음', '보통', '낮음'],
    ...
    return ListView(
      ...
      children: [
        _buildRadioGroup(
          ...
          ['깊은 수면', '빠른 수면', '숙면 지속'],
          sleepGoal,
          (v) => setState(() => sleepGoal = v),
        ),
        _buildRadioGroup(
          'Q25. 피드백 형태는?',
          ['텍스트 요약', '그래프', '음성으로 안내'],
          feedbackPreference,
          (v) => setState(() => feedbackPreference = v),
        ),
      ],
    );
  }
